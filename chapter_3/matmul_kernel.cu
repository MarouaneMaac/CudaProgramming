...
torch::Tensor matrix_multiplication(torch::Tensor matrix_1,torch::Tensor matrix_2){
    // Input dimensions
    const auto height_1 = matrix_1.size(0);
    const auto K = matrix_2.size(0);
    const auto width_2 = matrix_2.size(1);
    // Grid Params
    int block_size = 32;
    dim3 block_dim(block_size, block_size, 1);
    dim3 grid_dim((width_2 + block_size - 1) / block_size, (height_1 + block_size - 1) / block_size);
    // Init output
    torch::Tensor output = torch::empty({height_1, width_2}, matrix_1.options());
    // Launch Grid
